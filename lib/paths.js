...
// Vault paths — env resolution delegated to lib/vault-config (Slice 3.2:
// single source of truth; supports SKILL_ADVISOR_VAULT_PATH canonical and
// SKILL_ADVISOR_VAULT legacy with deprecation warning).
// Fallback chain: vault-config env → platform-specific Obsidian path → plugin-local vault dirs
const fs = require('fs');
const { resolveVaultPathFromEnv } = require('./vault-config');
