...
const { COMPLEXITY_BOUNDS } = require('./constants');
const { PIPELINE_OWNERS } = require('./pipeline-config');
...
function validateRouterOutput(input) {
  ...
  for (let i = 0; i < input.loadout.length; i++) {
    ...
    if (owner !== null && typeof owner === 'string' && !PIPELINE_OWNERS.includes(owner)) {
      errors.push(`loadout[${i}].pipeline_owner "${owner}" is not a recognized pipeline owner`);
    }
    owners.add(owner);
