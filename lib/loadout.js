...
const { PIPELINE_OWNERS, CANONICAL_FLOWS } = require('./pipeline-config');
...
/**
 * Replace the entry at a given zero-indexed position with a new entry
 * WITHOUT mutating the original loadout.
 *
 * Per premise 6 of the design doc: this is a LOCAL positional
 * substitution. `depends_on` arrays on other entries stay position-
 * indexed and are NOT rewritten — the replacement is assumed to satisfy
 * the same role as the original.
 *
 * Used by the gate's per-step picker when the user selects a same-owner
 * (or both-null) alternative for a specific position.
 *
 * @param {Array<object>} loadout - Current loadout (not mutated)
 * @param {number} position - Zero-indexed position to replace
 * @param {object} replacement - New entry, must at least carry an `invocation`
 * @returns {Array<object>} New loadout with position N replaced
 * @throws {Error} when position is out of bounds or replacement is malformed
 */
function swapAtPosition(loadout, position, replacement) {
  if (!Array.isArray(loadout)) {
    throw new TypeError('swapAtPosition: loadout must be an array');
  }
  if (!Number.isInteger(position) || position < 0 || position >= loadout.length) {
    throw new RangeError(
      `swapAtPosition: position ${position} is out of bounds for loadout of length ${loadout.length} (invalid position)`,
    );
  }
  if (!replacement || typeof replacement !== 'object' || Array.isArray(replacement)) {
    throw new TypeError('swapAtPosition: replacement must be an object');
  }
  if (typeof replacement.invocation !== 'string' || replacement.invocation.length === 0) {
    throw new Error('swapAtPosition: replacement must carry a non-empty invocation string');
  }
  return loadout.map((entry, i) => (i === position ? { ...replacement } : entry));
}
