...
function walk(dir, filename, results, depth, counter) {
  if (depth > 6 || counter.count > MAX_WALK_ENTRIES) return;
  let entries;
...
function main() {
  ...
  if (parseFloat(liteSize) > 100) {
    console.warn(`  WARNING: Lite index exceeds 100KB budget (${liteSize}KB). Hook cold-read may exceed 50ms.`);
  }
