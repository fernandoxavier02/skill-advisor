#!/usr/bin/env node
/** @typedef {import('./schemas').IndexEntry} IndexEntry */
/**
 * build-index.js � Scans all skill/plugin/MCP sources and generates
 * two JSON index files:
 *   - advisor-index-lite.json  (~30KB, for hook cold-read)
 *   - advisor-index-full.json  (~200KB, for /advisor command)
 */

const fs = require('fs');
const path = require('path');
const { PLUGIN_DIR, SKILL_DIR, getIndexPath, getLibDir } = require('./paths');
const { parseFrontmatter } = require('./frontmatter');
const { TRUNCATION, THRESHOLDS } = require('./constants');
const { debugLog } = require('./errors');
const { tagPipelineOwner } = require('./loadout');
// Internal callers MUST import inferCategory from ./category (not via the
// deprecated re-export below) so that the deprecation warning never fires
// for in-package use.
const { inferCategory } = require('./category');
const { walkDir } = require('./walk');

// -- Scanners -----------------------------------------------------
// findFilesRecursive is now a thin wrapper around the unified walker
// (lib/walk.js) introduced in Slice 3.1. Legacy semantics preserved:
// the previous private walker did NOT skip node_modules/.git, so we pass
// skipDirs=[] and skipHidden=false to keep parity for build-index callers
// that scan trees where those directories may legitimately contain
// SKILL.md / plugin.json files.
function findFilesRecursive(dir, exactFilename) {
  if (!fs.existsSync(dir)) return [];
  return walkDir(dir, {
    matcher: (name) => name === exactFilename,
    skipDirs: [],
    skipHidden: false,
  });
}

function scanSkills(dir, source) {
  const entries = [];
  const skillFiles = findFilesRecursive(dir, 'SKILL.md');
  for (const filePath of skillFiles) {
    try {
      const content = fs.readFileSync(filePath, 'utf8');
      const fm = parseFrontmatter(content);
      if (!fm || !fm.name) continue;
      const skillDir = path.basename(path.dirname(filePath));
      const entry = {
        id: `${source}:${fm.name}`,
        type: 'skill',
        source,
        name: fm.name,
        description: fm.description || '',
        invocation: `/${fm.name}`,
        category: inferCategory(fm.description || ''),
      };
      entry.pipeline_owner = tagPipelineOwner(entry);
      entries.push(entry);
    } catch (err) {
      debugLog('FS_READ', 'Failed to read skill file', { filePath, cause: err.message });
    }
  }
  return entries;
}

function scanPluginManifests() {
  const entries = [];
  if (!fs.existsSync(PLUGIN_DIR)) return entries;
  const manifestFiles = findFilesRecursive(PLUGIN_DIR, 'plugin.json');
  for (const filePath of manifestFiles) {
    try {
      const raw = fs.readFileSync(filePath, 'utf8');
      let manifest;
      try {
        manifest = JSON.parse(raw);
      } catch (err) {
        debugLog('PARSE_JSON', 'Failed to parse plugin.json', { filePath, cause: err.message });
        continue;
      }
      if (!manifest.name) continue;
      // plugin.json lives in .claude-plugin/ but content is in grandparent dir
      let pluginRoot = path.dirname(filePath);
      if (path.basename(pluginRoot) === '.claude-plugin') {
        pluginRoot = path.dirname(pluginRoot);
      }

      // Scan plugin skills
      const pluginSkillDir = path.join(pluginRoot, 'skills');
      if (fs.existsSync(pluginSkillDir)) {
        const skills = scanSkills(pluginSkillDir, `plugin:${manifest.name}`);
        entries.push(...skills);
      }

      // Scan plugin commands (with path traversal guard)
      if (Array.isArray(manifest.commands)) {
        for (const cmdPath of manifest.commands) {
          const cmdFile = path.resolve(pluginRoot, cmdPath);
          const rel = path.relative(pluginRoot, cmdFile);
          if (rel.startsWith('..') || path.isAbsolute(rel)) continue; // path traversal guard
          try {
            const content = fs.readFileSync(cmdFile, 'utf8');
            const fm = parseFrontmatter(content);
            const cmdName = fm?.name || path.basename(cmdPath, '.md');
            const cmdEntry = {
              id: `plugin:${manifest.name}:cmd:${cmdName}`,
              type: 'command',
              source: `plugin:${manifest.name}`,
              name: cmdName,
              description: fm?.description || '',
              invocation: `/${cmdName}`,
              category: inferCategory(fm?.description || ''),
            };
            cmdEntry.pipeline_owner = tagPipelineOwner(cmdEntry);
            entries.push(cmdEntry);
          } catch (err) {
            debugLog('FS_READ', 'Failed to read plugin command', { cmdFile, cause: err.message });
          }
        }
      }

      // Scan plugin agents (direct readdirSync, not recursive)
      const agentDir = path.join(pluginRoot, 'agents');
      if (fs.existsSync(agentDir)) {
        try {
          const agentEntries = fs.readdirSync(agentDir).filter(f => f.endsWith('.md'));
          for (const agentFile of agentEntries) {
            const fullPath = path.join(agentDir, agentFile);
            try {
              const content = fs.readFileSync(fullPath, 'utf8');
              const fm = parseFrontmatter(content);
              const agentName = fm?.name || path.basename(agentFile, '.md');
              const agentEntry = {
                id: `plugin:${manifest.name}:agent:${agentName}`,
                type: 'agent',
                source: `plugin:${manifest.name}`,
                name: agentName,
                description: fm?.description || '',
                invocation: agentName,
                category: inferCategory(fm?.description || ''),
              };
              agentEntry.pipeline_owner = tagPipelineOwner(agentEntry);
              entries.push(agentEntry);
            } catch (err) {
              debugLog('FS_READ', 'Failed to read plugin agent', { fullPath, cause: err.message });
            }
          }
        } catch (err) {
          debugLog('FS_READ', 'readdirSync failed for agents', { agentDir, cause: err.message });
        }
      }
    } catch (err) {
      debugLog('FS_READ', 'Failed to read plugin manifest', { filePath, cause: err.message });
    }
  }
  return entries;
}

const BANNED_KEYS = new Set(['__proto__', 'constructor', 'prototype']);

function scanMCPManifests() {
  const entries = [];
  const searchDirs = [PLUGIN_DIR];
  // Only scan cwd if it looks like a project (has .claude/ or .mcp.json)
  try {
    const cwd = process.cwd();
    if (fs.existsSync(path.join(cwd, '.claude')) || fs.existsSync(path.join(cwd, '.mcp.json'))) {
      searchDirs.push(cwd);
    }
  } catch (err) {
    debugLog('FS_READ', 'CWD check failed', { cause: err.message });
  }
  for (const dir of searchDirs) {
    const mcpFiles = findFilesRecursive(dir, 'mcp.json');
    for (const filePath of mcpFiles) {
      try {
        const stat = fs.statSync(filePath);
        if (stat.size > 1_000_000) continue; // skip files > 1MB
        const raw = fs.readFileSync(filePath, 'utf8');
        let manifest;
        try {
          manifest = JSON.parse(raw);
        } catch (err) {
          debugLog('PARSE_JSON', 'Failed to parse mcp.json', { filePath, cause: err.message });
          continue;
        }
        const servers = manifest.mcpServers || manifest;
        if (!servers || typeof servers !== 'object' || Array.isArray(servers)) continue;
        for (const [name, config] of Object.entries(servers)) {
          if (typeof config !== 'object' || BANNED_KEYS.has(name)) continue;
          entries.push({
            id: `mcp:${name}`,
            type: 'mcp',
            source: 'mcp',
            name: sanitize(name),
            description: sanitize(config.description || `MCP server: ${name}`),
            invocation: `mcp:${name}`,
            category: 'data',
            pipeline_owner: null,
          });
        }
      } catch (err) {
        debugLog('FS_READ', 'Failed to read mcp.json', { filePath, cause: err.message });
      }
    }
  }
  return entries;
}

function sanitize(str) {
  if (typeof str !== 'string') return '';
  // Strip ANSI escape codes and control chars
  return str.replace(/\x1b\[[0-9;]*[a-zA-Z]/g, '').replace(/[\x00-\x1f]/g, '');
}

// -- Category inference (DEPRECATED re-export) --------------------
//
// CATEGORY_PATTERNS and inferCategory now live in ./category. This module
// keeps a one-time-warning wrapper for back-compat with external callers
// that still do `require('./build-index').inferCategory(...)`. The wrapper
// will be removed in 0.6.0. Internal callers in this file import directly
// from ./category (see top of file) to avoid emitting the warning to
// ourselves.

let _deprecationWarned = false;

/**
 * DEPRECATED: import { inferCategory } from './category' instead.
 * Removed in 0.6.0. Emits a one-time console.warn per process.
 *
 * @param {string|null|undefined} description
 * @returns {string}
 */
function inferCategory_deprecated(description) {
  if (!_deprecationWarned) {
    _deprecationWarned = true;
    console.warn(
      '[skill-advisor] inferCategory re-exported from build-index is deprecated. ' +
      "Import from './category' instead. Removed in 0.6.0."
    );
  }
  return require('./category').inferCategory(description);
}

// -- Dedup --------------------------------------------------------

function dedup(entries) {
  const seen = new Map();
  for (const entry of entries) {
    const key = entry.id;
    if (!seen.has(key)) {
      seen.set(key, entry);
    }
  }
  return Array.from(seen.values());
}

// -- Build indexes ------------------------------------------------

function buildFullIndex(entries) {
  return entries;
}

function buildLiteIndex(entries) {
  return entries.map(e => ({
    id: e.id,
    name: e.name,
    description: truncate(e.description, TRUNCATION.LITE_DESC),
    invocation: e.invocation,
    category: e.category,
    pipeline_owner: e.pipeline_owner ?? null,
  }));
}

function truncate(str, maxLen) {
  if (typeof str !== 'string') return '';
  if (str.length <= maxLen) return str;
  return str.slice(0, maxLen).replace(/\s\S*$/, '...');
}

// -- Main ---------------------------------------------------------

function main() {
  const allEntries = [];

  // Global skills
  const globalSkills = scanSkills(SKILL_DIR, 'global');
  allEntries.push(...globalSkills);

  // Project skills (if in a project dir)
  const projectSkillDir = path.join(process.cwd(), '.claude', 'skills');
  if (fs.existsSync(projectSkillDir)) {
    const projectSkills = scanSkills(projectSkillDir, 'project');
    allEntries.push(...projectSkills);
  }

  // Plugin manifests (skills, commands, agents)
  const pluginEntries = scanPluginManifests();
  allEntries.push(...pluginEntries);

  // MCP servers
  const mcpEntries = scanMCPManifests();
  allEntries.push(...mcpEntries);

  // Dedup
  const unique = dedup(allEntries);

  // Build both indexes
  const full = buildFullIndex(unique);
  const lite = buildLiteIndex(unique);

  // Write
  const libDir = getLibDir();
  if (!fs.existsSync(libDir)) fs.mkdirSync(libDir, { recursive: true });

  const fullPath = getIndexPath('full');
  const litePath = getIndexPath('lite');

  fs.writeFileSync(fullPath, JSON.stringify(full, null, 2), 'utf8');
  fs.writeFileSync(litePath, JSON.stringify(lite, null, 2), 'utf8');

  const fullSize = (fs.statSync(fullPath).size / 1024).toFixed(1);
  const liteSize = (fs.statSync(litePath).size / 1024).toFixed(1);

  console.log(`Indexed ${unique.length} entries (${globalSkills.length} global skills, ${pluginEntries.length} plugin entries, ${mcpEntries.length} MCP servers)`);
  console.log(`  Full index: ${fullPath} (${fullSize}KB)`);
  console.log(`  Lite index: ${litePath} (${liteSize}KB)`);

  if (parseFloat(liteSize) > THRESHOLDS.LITE_INDEX_BUDGET_KB) {
    console.warn(`  WARNING: Lite index exceeds ${THRESHOLDS.LITE_INDEX_BUDGET_KB}KB budget (${liteSize}KB). Hook cold-read may exceed 50ms.`);
  }

  return { full, lite, stats: { total: unique.length, globalSkills: globalSkills.length, pluginEntries: pluginEntries.length, mcpEntries: mcpEntries.length } };
}

if (require.main === module) {
  main();
}

module.exports = {
  main,
  parseFrontmatter,
  // DEPRECATED: re-export shim. Removed in 0.6.0. Import from './category'.
  inferCategory: inferCategory_deprecated,
  dedup,
  scanSkills,
  buildLiteIndex,
  buildFullIndex,
  truncate,
};
