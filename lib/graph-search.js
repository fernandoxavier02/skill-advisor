...
const { GRAPH_PARAMS } = require('./constants');
const { normalizeAccents } = require('./text');

const { SCORE_BY_HOP, CONVERGENCE_BOOST, CATEGORY_BOOST } = GRAPH_PARAMS;
...
// ---------------------------------------------------------------------------
// normalizeToken — alias for normalizeAccents from lib/text.js
// ---------------------------------------------------------------------------
const normalizeToken = normalizeAccents;
...
function loadGraph(graphDir) {
  ...
  let stat;
  try {
    stat = fs.statSync(adjacencyPath);
  } catch (err) {
    if (err.code === 'ENOENT') {
      throw new Error('Graph not found. Run /advisor-catalog first to build the vault graph.');
    }
    throw err;
  }
  ...
  let raw, graph;
  try {
    raw = fs.readFileSync(adjacencyPath, 'utf8');
    graph = JSON.parse(raw);
  } catch (err) {
    throw new Error(`Failed to parse adjacency.json: ${err.message}`);
  }
