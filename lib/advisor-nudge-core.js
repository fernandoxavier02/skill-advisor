...
function runNudge(opts) {
  ...
  // contextMod tri-state with explicit `false` sentinel for "disable":
  //   undefined / null  → lazy-load real ./context module (default)
  //   false             → DISABLE branch context boost entirely (sentinel)
  //   <object>          → use as-is (test injection)
  // Adversarial review consensus: passing `null` to disable was a foot-gun;
  // explicit `false` makes intent unambiguous.
  const contextMod = o.contextMod;
  ...
  // Layer 2 — keyword (cap iteration to bound hot-path cost)
  let kwCount = 0;
  for (const entry of index) {
    const score = scoreEntry(promptTokens, entry);
    if (score >= scoreThreshold) {
      keywordScores.set(entry.id, score);
      if (++kwCount >= SW.KEYWORD_CAP) break;
    }
  }
  ...
  if (hookData && Array.isArray(hookData.affinity)) {
    ...
    for (const entry of scored) {
      ...
      if (affinity) entry.score = Math.min(1.0, entry.score + affinity * BOOST_FACTORS.AFFINITY);
    }
  }
  ...
  let ctx = contextMod;
  if (ctx === false) {
    ctx = null; // explicit disable sentinel
  } else if (ctx === undefined || ctx === null) {
    try { ctx = require('./context'); } catch { ctx = null; }
  }
  if (ctx && typeof ctx.getBranchCategory === 'function') {
    try {
      ...
      if (branchCategory) {
        for (const entry of scored) {
          if (entry.category === branchCategory) entry.score = Math.min(1.0, entry.score + BOOST_FACTORS.CONTEXT);
        }
      }
    } catch (err) {
  ...
  const matches = top
    .map(e => {
      const rawInvocation = String(e.invocation || '');
      const inv = rawInvocation.replace(/[^a-zA-Z0-9:/_-]/g, '').slice(0, 60);
      if (!inv) return null;
  ...
  if (hookData && Array.isArray(hookData.replay) && hookData.replay.length > 0) {
    ...
    for (const candidate of hookData.replay) {
      ...
      if (topBareNames.has(firstSkill)) {
        // Defense-in-depth: sanitize each sequence member with the same allowlist
        // used for invocation, and coerce count to a finite integer. Sequence
        // members are bare skill names from local telemetry, so the risk is low
        // — this hardens against future telemetry-format drift.
        const safeSeq = candidate.sequence
          .map(s => String(s).replace(/[^a-zA-Z0-9:/_-]/g, '').slice(0, 60))
          .filter(Boolean);
        if (safeSeq.length === 0) break;
        const safeCount = Number.isFinite(candidate.count) ? Math.floor(candidate.count) : 0;
        const seqStr = safeSeq.join(' → ');
        output.push(`[Advisor] Pipeline anterior: ${seqStr} (usado ${safeCount}x). Rode /advisor para replay.`);
        break;
      }
    }
  }
